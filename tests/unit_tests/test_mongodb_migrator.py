...
class Test:
	...
	def __init__(self, url, database_name, collection_name, keyspace_name, table_name, primary_key, flatten=False, client_url=None, cred_file=None, secure_bundle=None, token=None):
		...
		self.mongo_to_cassandra(database_name=self.database_name, collection_name=self.collection_name, client_url=self.client_url, cred_file=self.cred_file, keyspace_name=self.keyspace_name, table_name=self.table_name, primary_key=self.primary_key, secure_bundle=self.secure_bundle, token=self.token, flatten=flatten)
	...
	def mongo_to_cassandra(self, database_name, collection_name, keyspace_name, table_name, primary_key='id', flatten=False, client_url=None, cred_file=None, secure_bundle=None, token=None):
		self.handler.mongo_to_cassandra(primary_key=primary_key, client_url=self.client_url, cred_file=cred_file, database_name=self.database_name, collection_name=self.collection_name, keyspace_name=keyspace_name, table_name=table_name, secure_bundle=secure_bundle, token=token, flatten=flatten)
		assert True